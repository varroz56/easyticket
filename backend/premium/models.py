...
class PremiumPackage(models.Model):
    ...
    def create_unique_slug(self):
        ...
        if ppackages:
            print("Found same slug start")
            if len(ppackages) == 1:
                print("Only one same slug start: " + ppackages.first().slug)
                if ppackages.first().slug == myslug:
                    myslug += "-1"
                    ...
                    print(
                        "Found same slug start but not same slug, using slugified name")
                    PremiumPackage.objects.filter(
                        id=self.id).update(slug=myslug)
                    print("Set instance slug to: " + myslug)
                ...
                print("Number of same slug starts:")
                print(len(ppackages))
                n = 1
